...
fn test_convert_import() {
    let stmt = Stmt::Import { module: "math".to_string(), alias: None, selections: None, is_unsafe: false, position: dummy_pos() };
    let (graph, root) = convert_program(vec![stmt]);
    let elements = graph.get_ordered_edges(root, "Element");
...
fn test_convert_import_with_alias() {
    let stmt = Stmt::Import { module: "math".to_string(), alias: Some("m".to_string()), selections: None, is_unsafe: false, position: dummy_pos() };
    let (graph, root) = convert_program(vec![stmt]);
    let elements = graph.get_ordered_edges(root, "Element");
    let import = graph.get_node(elements[0]).unwrap();
