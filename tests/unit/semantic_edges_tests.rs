...
#[test]
fn test_getter_creates_depends_on_edges() {
    // A getter that reads properties should create depends_on edges
    let code = r#"
        graph Rectangle {
            width: 10
            height: 20
    ...
    if let ValueKind::List(edge_list) = &edges.kind {
        let edges_vec = edge_list.to_vec();
        // area should have depends_on edges to width and height
        let has_width_dep = edges_vec.iter().any(|edge| {
            if let ValueKind::List(edge_parts) = &edge.kind {
                let parts = edge_parts.to_vec();
                if parts.len() >= 3 {
                    let from = get_string(&parts[0]).unwrap_or_default();
                    let to = get_string(&parts[1]).unwrap_or_default();
                    let edge_type = get_string(&parts[2]).unwrap_or_default();
                    return from == "area" && to == "width" && edge_type == "depends_on";
                }
            }
            false
        });
        assert!(has_width_dep, "area should depend on width. Edges: {:?}", edges_vec);
    } else {
        panic!("edges should be a list");
    }
}
...
#[test]
fn test_dependencies_returns_property_list() {
    // dependencies("prop") returns what this property depends on
    let code = r#"
        graph Rectangle {
            width: 10
            height: 20
    ...
    let deps = exec.env().get("deps").expect("Should have deps");
    if let ValueKind::List(dep_list) = &deps.kind {
        let props: Vec<String> = dep_list.to_vec().iter()
            .filter_map(|v| {
                if let ValueKind::String(s) = &v.kind {
                    Some(s.clone())
                } else {
            ...
            .collect();
        assert!(props.contains(&"width".to_string()), "area depends on width");
        assert!(props.contains(&"height".to_string()), "area depends on height");
    } else {
        panic!("dependencies should return a list");
    }
}

#[test]
fn test_dependents_returns_computed_props() {
    // dependents("prop") returns computed properties that depend on this
    let code = r#"
        graph Rectangle {
            width: 10
            height: 20
    ...
    if let ValueKind::List(dep_list) = &deps.kind {
        let props: Vec<String> = dep_list.to_vec().iter()
            ...
            .collect();
        assert!(props.contains(&"area".to_string()), "area depends on width");
        assert!(props.contains(&"perimeter".to_string()), "perimeter depends on width");
    } else {
        panic!("dependents should return a list");
    }
}

#[test]
fn test_dependency_order_topological() {
    // dependency_order() returns properties in topological order
    let code = r#"
        graph Rectangle {
            width: 10
            height: 20
    ...
    let order = exec.env().get("order").expect("Should have order");
    if let ValueKind::List(order_list) = &order.kind {
        let props: Vec<String> = order_list.to_vec().iter()
            .filter_map(|v| {
                if let ValueKind::String(s) = &v.kind {
                    Some(s.clone())
                } else {
            ...
            .collect();
        // width and height should come before area
        let width_pos = props.iter().position(|p| p == "width");
        let height_pos = props.iter().position(|p| p == "height");
        let area_pos = props.iter().position(|p| p == "area");

        if let (Some(w), Some(h), Some(a)) = (width_pos, height_pos, area_pos) {
            assert!(w < a, "width should come before area");
            assert!(h < a, "height should come before area");
        } else {
            panic!("All properties should be in dependency order");
        }
    } else {
        panic!("dependency_order should return a list");
    }
}
