...
impl Parser {
    ...
    // ========================================================================
    // Phase 9: Graph Pattern Parsing
    // ========================================================================

    /// Parse graph pattern: (node:Type) -[:EDGE]-> (other:Type)
    /// Called when parsing arguments to .match() method
    fn parse_graph_pattern(&mut self) -> Result<crate::ast::GraphPattern> {
        use crate::ast::{GraphPattern, PatternNode, PatternEdge};
        ...
        while self.check(&TokenType::Minus) {
            let edge = self.parse_pattern_edge(&nodes, edges.len())?;
        ...
        if nodes.is_empty() {
            return Err(GraphoidError::SyntaxError {
                message: "Graph pattern must have at least one node".to_string(),
                position,
            });
        }

        // No where/return clauses parsed here - they're chained method calls
        Ok(GraphPattern {
            nodes,
            edges,
            where_clause: None,
    ...
    /// Parse pattern node: (variable:Type) or (variable)
    fn parse_pattern_node(&mut self) -> Result<crate::ast::PatternNode> {
        ...
        if !self.match_token(&TokenType::LeftParen) {
            return Err(GraphoidError::SyntaxError {
                message: "Expected '(' to start pattern node".to_string(),
                position,
            });
        }
        ...
        let variable = match &self.peek().token_type {
            ...
            _ => {
                return Err(GraphoidError::SyntaxError {
                    message: "Expected identifier for pattern node variable".to_string(),
                    position: self.peek().position(),
                });
            }
        };
        ...
        // Note: Lexer treats :Type as a Symbol token, not Colon + Identifier
        let node_type = match &self.peek().token_type {
            TokenType::Symbol(sym) => {
                // Extract type name from symbol (remove leading :)
                let type_name = sym.strip_prefix(':').unwrap_or(sym).to_string();
                self.advance();
                Some(type_name)
            }
            _ => None,
        ...
        if !self.match_token(&TokenType::RightParen) {
            return Err(GraphoidError::SyntaxError {
                message: "Expected ')' after pattern node".to_string(),
                position: self.peek().position(),
            });
        }
    ...
    /// Parse pattern edge: -[:TYPE]-> or -[:TYPE*min..max]-> or -[:TYPE]-
    fn parse_pattern_edge(&mut self, nodes: &[crate::ast::PatternNode], edge_index: usize) -> Result<crate::ast::PatternEdge> {
        use crate::ast::{PatternEdge, EdgeDirection, EdgeLength};
        ...
        if !self.match_token(&TokenType::Minus) {
            return Err(GraphoidError::SyntaxError {
                message: "Expected '-' to start edge pattern".to_string(),
                position,
            });
        }

        // Parse optional edge type: [:TYPE] or [:TYPE*min..max]
        // Note: Lexer treats :TYPE as a Symbol token, not Colon + Identifier
        let (edge_type, length) = if self.match_token(&TokenType::LeftBracket) {
            // Parse edge type from Symbol token
            let edge_type = match &self.peek().token_type {
                TokenType::Symbol(sym) => {
                    // Extract type name from symbol (remove leading :)
                    let type_name = sym.strip_prefix(':').unwrap_or(sym).to_string();
                    self.advance();
                ...
                _ => {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected edge type (e.g., [:FRIEND]) in edge pattern".to_string(),
                        position: self.peek().position(),
                    });
                }
            };
            ...
            let length = if self.match_token(&TokenType::Star) {
                // Expect {
                if !self.match_token(&TokenType::LeftBrace) {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected '{' after '*' in variable-length path".to_string(),
                        position: self.peek().position(),
                    });
                }

                // Parse min: N
                if !matches!(&self.peek().token_type, TokenType::Identifier(id) if id == "min") {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected 'min:' in variable-length path".to_string(),
                        position: self.peek().position(),
                    });
                }
                self.advance();
                ...
                if !self.match_token(&TokenType::Colon) {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected ':' after 'min'".to_string(),
                        position: self.peek().position(),
                    });
                }

                let min = match &self.peek().token_type {
                    TokenType::Number(n) => {
                    ...
                    _ => {
                        return Err(GraphoidError::SyntaxError {
                            message: "Expected number after 'min:'".to_string(),
                            position: self.peek().position(),
                        });
                    }
                };
                ...
                if !self.match_token(&TokenType::Comma) {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected ',' after min value".to_string(),
                        position: self.peek().position(),
                    });
                }

                // Parse max: M
                if !matches!(&self.peek().token_type, TokenType::Identifier(id) if id == "max") {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected 'max:' in variable-length path".to_string(),
                        position: self.peek().position(),
                    });
                }
                self.advance();
                ...
                if !self.match_token(&TokenType::Colon) {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected ':' after 'max'".to_string(),
                        position: self.peek().position(),
                    });
                }

                let max = match &self.peek().token_type {
                    TokenType::Number(n) => {
                        let max_val = *n as usize;
                        self.advance();
                        max_val
                    }
                    _ => {
                        return Err(GraphoidError::SyntaxError {
                            message: "Expected number after 'max:'".to_string(),
                            position: self.peek().position(),
                        });
                    }
                };
                ...
                if !self.match_token(&TokenType::RightBrace) {
                    return Err(GraphoidError::SyntaxError {
                        message: "Expected '}' after max value".to_string(),
                        position: self.peek().position(),
                    });
                }
            ...
            if !self.match_token(&TokenType::RightBracket) {
                return Err(GraphoidError::SyntaxError {
                    message: "Expected ']' after edge type".to_string(),
                    position: self.peek().position(),
                });
            }

            (Some(edge_type), length)
        } else {
            (None, EdgeLength::Fixed)
        };

        // Cypher syntax is either -[:TYPE]-> (directed) or -[:TYPE]- (bidirectional)
        // We've consumed the first '-' and optionally '[:TYPE]'
        // Now expect the second '-'
        if !self.match_token(&TokenType::Minus) {
            return Err(GraphoidError::SyntaxError {
                message: "Expected '-' after edge pattern (use -[:TYPE]-> or -[:TYPE]-)".to_string(),
                position: self.peek().position(),
            });
        }

        // Parse direction: check for '>' to make it directed
        let direction = if self.match_token(&TokenType::Greater) {
            EdgeDirection::Directed
        } else {
            EdgeDirection::Bidirectional
        };

        // Set from/to based on node order
        let from = nodes[edge_index].variable.clone();
        let to = if nodes.len() > edge_index + 1 {
            nodes[edge_index + 1].variable.clone()
        } else {
            // Will be filled by next node
            String::new()
        };
