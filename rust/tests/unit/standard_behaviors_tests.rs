...
fn test_none_to_zero_transforms_none() {
    ...
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::NoneToZero)];
...
fn test_none_to_empty_transforms_none() {
    // none → ""
    let value = Value::None;
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::NoneToEmpty)];
...
fn test_positive_makes_negative_positive() {
    ...
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::Positive)];
...
fn test_round_to_int_rounds_numbers() {
    // 3.7 → 4.0, 3.2 → 3.0
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::RoundToInt)];

    let result = apply_behaviors(Value::Number(3.7), &behaviors).unwrap();
    assert_eq!(result, Value::Number(4.0));

    let result = apply_behaviors(Value::Number(3.2), &behaviors).unwrap();
...
fn test_uppercase_converts_string() {
    ...
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::Uppercase)];
...
fn test_lowercase_converts_string() {
    // "HELLO" → "hello"
    let value = Value::String("HELLO".to_string());
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::Lowercase)];
...
fn test_validate_range_clamps_numbers() {
    // 110 → 100 (range 0-100)
    // -10 → 0 (range 0-100)
    let behaviors = vec![BehaviorInstance::new(BehaviorSpec::ValidateRange {
        min: 0.0,
        max: 100.0,
    })];

    let result = apply_behaviors(Value::Number(110.0), &behaviors).unwrap();
    assert_eq!(result, Value::Number(100.0));

    let result = apply_behaviors(Value::Number(-10.0), &behaviors).unwrap();
    assert_eq!(result, Value::Number(0.0));
}
...
fn test_validate_range_within_range() {
    ...
    let result = apply_behaviors(Value::Number(50.0), &behaviors).unwrap();
    assert_eq!(result, Value::Number(50.0));

    // Edge values
    let result = apply_behaviors(Value::Number(0.0), &behaviors).unwrap();
    assert_eq!(result, Value::Number(0.0));

    let result = apply_behaviors(Value::Number(100.0), &behaviors).unwrap();
    assert_eq!(result, Value::Number(100.0));
}
...
fn test_multiple_behaviors_chain() {
    ...
    let behaviors = vec![
        ...
        BehaviorInstance::new(BehaviorSpec::RoundToInt),
    ];
...
fn test_behavior_order_matters() {
    ...
    let behaviors = vec![
        BehaviorInstance::new(BehaviorSpec::NoneToZero),
        BehaviorInstance::new(BehaviorSpec::ValidateRange { min: 5.0, max: 10.0 }),
    ];
...
fn test_list_with_none_to_zero() {
    // List of [none, 1, none] with add_behavior should transform to [0, 1, 0]
    // This test requires add_behavior() method to be implemented
    ...
    list.add_behavior(behavior).unwrap();
...
fn test_hash_with_uppercase() {
    // Hash values uppercased with add_behavior
    ...
    // Add behavior - should transform retroactively
    let behavior = BehaviorInstance::new(BehaviorSpec::Uppercase);
    hash.add_behavior(behavior).unwrap();
...
fn test_proactive_on_append() {
    ...
    let behavior = BehaviorInstance::new(BehaviorSpec::Positive);
    list.add_behavior(behavior).unwrap();
