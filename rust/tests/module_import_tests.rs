...
fn test_import_creates_module_namespace() {
    ...
    let test_file = PathBuf::from("test_data/modules/test.gr");
    executor.set_current_file(Some(test_file));

    executor.execute_source(source).unwrap();

    // Verify module exists in environment
    let module_value = executor.get_variable("simple_module");
    assert!(module_value.is_some(), "Module 'simple_module' should be in environment");
}
...
fn test_access_module_variable() {
    ...
    let source = r#"
        import "simple_module"
        value = simple_module.helper_value
    "#;
